...
(function() {
	...
	function init() {
		...
		if(v && v >= requiredVersion) { 
			...
			yepnope({
				load: 'http://localhost/trackerv2/js/neon.js',
				callback: function(){ console.log("neon load");}
			});
		} else {
			//load our jquery
			yepnope({
				...
				load: ['http://ajax.googleapis.com/ajax/libs/jquery/1.11.0/jquery.min.js', 'http://localhost/trackerv2/js/neon.js'],
				//callback: function() {
				//	//jquery loaded
				...
				//}
				callback: {
					"jQ" : function() { _neonjQuery = $.noConflict(true); console.log("JQ loaded"); },
					"neon": function() { console.log("neon loaded");}
				}
